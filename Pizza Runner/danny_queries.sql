...
(
   ...
   JOIN sales
		ON members.customer_id = sales.customer_id
   WHERE 
		order_date >= join_date AND order_date <= '2021-01-31'
)
SELECT customer_id, 
	#	order_date,
    #    join_date,
    #    valid_date,
	SUM( CASE 
	  WHEN order_date BETWEEN dates.join_date AND dates.valid_date THEN menu.price*20
...
FROM dates
JOIN menu
	ON dates.product_id = menu.product_id
